...
class Optimizer(object):
    ...
    def define_lr_scheduler(self, optimizer, milestones):
        if self.optim_dict["optimizer"] in ['SGD', 'Adam']:
            ...
            max_epoch = self.optim_dict.get('num_epoch', 80) 
            warmup_epoch = self.optim_dict.get('warmup_epoch', 5) 

            def lr_lambda(epoch):
                if epoch < warmup_epoch:
                    return float(epoch + 1) / float(max(1, warmup_epoch))
                progress = float(epoch - warmup_epoch) / float(max(1, max_epoch - warmup_epoch))
